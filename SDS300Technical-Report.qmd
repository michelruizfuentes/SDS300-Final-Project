...
The replication crisis refers to a lack of reproducibility in experimental designs and the scientific method. For example, say one investigator is trying to repeat the research design of another scientist, but they cannot produce the same results as the original study. The replication crisis occurs due to inconsistencies and manipulations that are not reported throughout the study. As a result, they produce misleading and sometimes favorable results that are not indicative of the true results of the experiment. However, to solve this issue, researchers can pre-register their study. Preregistration refers to a set of protocols that support "open science," which is reproducible, accessible, transparent, and rigorous. Overall these "open science" principles allow for best and ethical practices for research.

Preregistration consists of a list of criteria that help mitigate the threats of unethical scientific practices and encourage the responsible conduct of research. In the following paragraph, I will list some preregistration strategies and describe how they address the replication crisis. First, by consolidating the research methodology and process before the experiment, the researcher is less likely to change their methods to skew their results because they are following the experiment sequence they designed beforehand. Second, by conducting background research about the theory to develop a hypothesis, the author uses literature to develop a prediction and will not create their hypothesis after running the results; otherwise, their predictions will always be true. Third, having a public repository of the data and code for the analyses allows for the accessibility and reproducibility of the experiment. Fourth, by discussing model limitations and condition checks, such as whether random sampling and random assignment were achieved for this study, the investigator are transparent about their data and the application of their findings. And lastly, by emphasizing the substantive importance of the results in addition to the statistical interpretation and significance, they have demonstrated a rigorous and comprehensive understanding of their results.
...
In a t-test, the researcher determines if there is a significant difference between the means of the two groups. This test calculates a t-value and compares this to the critical value to show if the difference is statistically significant. I will use a t-test to assess if there is a statistically significant difference in the frequency rate of choosing Option B (`bRate`) dependent on the information (`Amb`) or feedback (`Feedback`) provided. Therefore, if there is no significant difference, then `Amb` and `Feedback` do not have statistically significant explanatory power. In other words, the `bRate` will not change based on the information (`Amb`) or feedback (`Feedback`) provided. 

### [One-Way ANOVA:]{.underline}

### [Box plot:]{.underline}

Because I will a t-test to compare group means, the box plot will help visualize the distribution of the data and any potential outliers or skewness. It will also be used to demonstrate the distribution of probabilities for Gambles A and B, with separate boxes for each. Use box plots to visualize the distribution of the outcome variable (`bRate`) across different levels of a categorical variable (e.g., `feedback`, `Amb`) to check for differences in means.
...
Levene's test is designed to test a hypothesis, so a large p-value indicates that we fail to reject the null and the groups have homogeneity. This is evidence that the population variances are not significantly different, and we have constant variance among the groups.
...
Use histograms to visualize the distribution of each variable (`Ha`, `Hb`, `pHa`, `pHb`, `bRate`, `Lb`, `LotShapeB`, `LotNumB`) to check for normality assumptions.
...
```{r, warning=F, message=F}
# Assuming you have a logistic regression model "logit_model" predicting the binary outcome variable "choice"
...
### [**Probability Distribution:**]{.underline}

```{r, echo = FALSE, warning=F, message=F}
prob_pHa <-
  ggplot(data=choices_df, aes(x=pHa)) + 
  geom_histogram(bins =10) + 
  labs(title ="Probability of the First Outcome",
      subtitle= "Distribution Plot of Option A",
      caption ="source: Choices13K") +
  xlab("Probability in Option A (0-impossible, 1-certain)") +
  ylab ("Occurances Count") 
prob_pHa
```

```{r, echo = FALSE, warning=F, message=F}
prob_La <-
  ggplot(data=choices_df, aes(x=La)) + 
  geom_histogram(bins =10) + 
  labs(title ="Probability of the Second Outcome",
      subtitle= "Distribution Plot of Option A",
      caption ="source: Choices13K") +
  xlab("Probability in Option A") +
  ylab ("Occurances Count") 
prob_La
```
...
choicepHa_histogram <- ggplot(choices_df, aes(x=pHa)) +
  geom_histogram(bins =20) +
  ggtitle ("Distribution of 1st Outcome in Option A") +
  xlab ("Probability of Occurence  (0- will not | 1- certain)") +
  ylab ("Count of Outcome 1 in A")

choicepHa_histogram
```

```{r, echo = FALSE, warning=F, message=F}
# Boxplot of pHa by feedback
pHa_boxplot <- 
  boxplot(pHa ~ Feedback, data=choices_df, 
          main="Boxplot of pHa by Feedback", 
          xlab="Feedback", ylab="pHa")
pHa_boxplot

summary(choices_df$pHa)
```

[**Option B:**]{.underline} *Histograms: Use histograms to visualize the distribution of each variable (Ha, Hb, pHa, pHb, bRate, Lb, LotShapeB, LotNumB) to check for normality assumptions.*

```{r, echo = FALSE, warning=F, message=F}
prob_pHb <-
  ggplot(data=choices_df, aes(x=pHb)) + 
  geom_histogram(bins =10) + 
  labs(title ="Probability of the First Outcome",
      subtitle= "Distribution Plot of Option B",
      caption ="source: Choices13K") +
  xlab("Probability in Option B (0-impossible, 1-certain)") +
  ylab ("Occurances Count") 
prob_pHb
```
...
```{r, echo = FALSE, warning=F, message=F}
prob_Lb <-
  ggplot(data=choices_df, aes(x=Lb)) + 
  geom_histogram(bins =10) + 
  labs(title ="Probability of the Second Outcome",
      subtitle= "Distribution Plot of Option B",
      caption ="source: Choices13K") +
  xlab("Probability in Option B") +
  ylab ("Occurances Count") 
prob_Lb
```
...
choiceB_histogram <- ggplot(choices_df, aes(x=pHb)) +
  geom_histogram(bins =20) +
  ggtitle ("Histogram of the Probability of Option B") +
  xlab ("Probability of Occurence  (0- will not, 1- certain)") +
  ylab ("Count")
...
```{r, echo = FALSE, warning=F, message=F}
# Boxplot of pHb by LotShapeB
pHb_boxplot <- 
  boxplot(pHb ~ LotShapeB, data=choices_df, 
          main="Boxplot of pHb by LotShapeB", 
          xlab="LotShapeB", ylab="pHb")
pHb_boxplot
```
