...
During this stage of data preprocessing, I checked for missing values, removing, or correcting any errors/inconsistencies, and data types. However, because this data was used for a machine-learning project, the data is relatively accessible and clean, and I will be using it for statistics and results purposes.  
...
Use heat maps to visualize the relationship between two categorical variables (e.g., feedback and the chosen Gamble) to check for any patterns or associations.

### [**Correlation Matrix Plot:**]{.underline} 

Use a correlation matrix plot to visualize the pairwise correlations between all continuous variables in the data set to check for multicollinearity.
...
```{r, warning=F, message=F}
# # Assuming you have a data frame "df" with several continuous variables
# # Use the "cor" function to calculate the pairwise correlations between variables
# corr_matrix <- cor(df)
# 
...
# feedback_model <- glm(Feedback ~ pHa, data = choices_df, family = binomial())
# 
# # create logistic regression plot
# ggplot(choices_df, aes(x = pHa, y = Feedback)) +
#   geom_point(aes(color = as.factor(Feedback)), size = 2.5) +
#   stat_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, size = 1.2, color = "black") +
#   scale_color_manual(values = c("#00BFC4", "#F8766D")) +
#   theme_minimal()
```

```{r, echo=FALSE, warning=F, message=F}
# # Fit a logistic regression model to pred  ict choice (0 = A, 1 = B) based on Ha
# pHa_model <- glm(Feedback ~ pHa, data = choices_df, family = binomial())
# # Print the model summary
# summary(pHa_model)
```

```{r, echo=FALSE, warning=F, message=F}
# # Fit a logistic regression model to predict choice (0 = A, 1 = B) based on Ha
# pHb_model <- glm(Amb~ pHb, data = choices_df, family = binomial())
# # Print the model summary
# summary(pHb_model)
```
